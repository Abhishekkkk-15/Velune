...
export const api = {
  ...
  getArtist: (id: string) =>
    get<{ id: string; name: string; thumbnail: string; description?: string; sections: { title: string; items: YTItem[] }[] }>(`/artist/${id}`),

  getAlbum: (id: string) =>
    get<{ id: string; title: string; artists: any[]; year?: string; thumbnail: string; songs: YTTrack[] }>(`/album/${id}`),

  getPlaylist: (id: string) =>
    get<{ id: string; title: string; thumbnail: string; songs: YTTrack[] }>(`/playlist/${id}`),
  ...
  getMoods: () =>
    get<{ moods: { title: string; color: string; params?: string; browseId?: string }[] }>('/moods'),

  getCharts: () =>
    get<{ sections: HomeSection[] }>('/charts'),
  ...
  getDownloadStatus: (videoId: string) =>
    get<{ status: 'not_started' | 'pending' | 'downloading' | 'done' | 'error' }>(`/download/${videoId}/status`),
  ...
  lastfmNowPlaying: (data: { sessionKey: string; artist: string; track: string; album?: string; duration?: number; apiKey?: string; apiSecret?: string }) =>
    post('/lastfm/now-playing', data),

  lastfmScrobble: (data: { sessionKey: string; artist: string; track: string; album?: string; duration?: number; timestamp: number; apiKey?: string; apiSecret?: string }) =>
    post('/lastfm/scrobble', data),

  discordActivity: (data: { title: string; artist: string; album?: string; thumbnail?: string; startTimestamp?: number }) =>
